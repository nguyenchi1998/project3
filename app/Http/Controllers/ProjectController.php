...
class ProjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Project::when(
            ...
            ->when(
                isset($filters['type']), function ($query) use ($filters) {
                    $query->where('type', $filters['type']);
                }
            )
            ->when(
                isset($filters['keyword']), function ($query) use ($filters) {
    ...
    public function trackerIssuesStatistic($id)
    {
        ...
        return array_map(
            function ($tracker) {
                $issues = array_reduce(
                    $tracker['issues'], function ($totalIssue, $issue) {
    ...
    public function priorityIssuesStatistic($id, Request $request)
    {
        ...
        $data = Issue::where('project_id', $id)
            ->when(
                isset($filters['trackerId']), function ($query) use ($filters) {
    ...
    public function getMembers(Project $project, Request $request)
    {
        ...
        $project
            ->load(
                ['members' => function ($query) use ($filters) {
                    $query->when(
                        isset($filters['keyword']), function ($query) use ($filters) {
                            $query->where('name', 'like', '%' . $filters['keyword'] . '%');
                        }
    ...
    public function getIssues($id, FilterIssueRequest $request)
    {
        ...
        return Issue::where('project_id', $id)
            ->when(
                isset($ignoreIds), function ($query) use ($ignoreIds) {
                    $query->whereNotIn('id', $ignoreIds);
                }
            )
            ->when(
                count($filters), function ($query) use ($filters) {
                    $query->where(
                        function ($query) use ($filters) {
                            $query->when(
                                ...
                                isset($filters['assigneeId']), function ($query) use ($filters) {
                                    $query->where('assign_user_id', $filters['assigneeId']);
                                }
                            )->when(
                                isset($filters['authorId']), function ($query) use ($filters) {
                                ...
                                isset($filters['status']) && $filters['status'] != 'all', function ($query) use ($filters) {
                                    $query->where('status', $filters['status']);
                                }
                            )->when(
                                isset($filters['priority']) && $filters['priority'] != 'all', function ($query) use ($filters) {
                                    $query->where('priority', $filters['priority']);
                                }
                            )->when(
                                isset($filters['startDate']), function ($query) use ($filters) {
                                    $query->whereDate('start_date', '>=', $filters['startDate']);
                                }
                            )->when(
                                isset($filters['dueDate']), function ($query) use ($filters) {
    ...
    public function getMemberActivities($projectId, $memberId, Request $request)
    {
        return IssueHistory::where('updated_user_id', $memberId)
            ->whereHas(
                'issue', function ($query) use ($projectId) {
                    $query->where('project_id', $projectId);
                }
            )
